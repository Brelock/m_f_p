...
const rtloptions = {
  ...
  "stringMap": [
    ...
    },
    {
      ...
      "replace"  : ["right", "Left", "LEFT"],
    },
    {
      "name"    : "flex-start-flex-end",
      "priority": 1000,
      "search"  : ["flex-start", "Start", "START"],
      "replace" : ["flex-end", "End", "END"],
      "options" : {
          "scope" : "*",
          "ignoreCase" : false
        }
...
gulp.task('build:css', function () {
    return gulp.src([path.src.scss, path.src.css]) 
        ...
        // .pipe(rtlcss({options: rtloptions})) // Convert to RTL.
...
gulp.task('purgecss', () => {
  return gulp.src(['production/css/main.min.css','production/css/main.min.rtl.css', path.src.purgingcss])
       .pipe(
            purgecss({
                content: ['production/*.html'],
                whitelistPatterns: [/\bjs_/, /active/, /\bslick-/, /\bel-/, /\bicon-/, /\bchosen-/, /\bcbox/, /colorbox/, /\blombard/, /\bgm/, /about-content/]
            })
        )
