var ToggleContentModule  = (function() {
	...
	function thisContentTarget(buttonElement, buttonData) {
		let parent = $(buttonElement).closest('.accordionGroupParent');
		let thisTarget;
		if (parent.length) {
			thisTarget = buttonData.group ? parent[0].getElementsByClassName(buttonData.group) : null;
		} else {
			thisTarget = buttonData.group ? buttonElement.parentElement.getElementsByClassName(buttonData.group) : null;	
		}
		return thisTarget;
	}
