...
var path = {
    ...
    src: { //patch for source files
        html: 'src/*.html', 
        js: 'src/js/index.js',
        departmentsMapjs: 'src/js/libs/departmentsMap.js',
    ...
    buildForSite: {
        js: '/var/www/html/technostock.loc/public/js/site/',
        css: '/var/www/html/technostock.loc/resources/styles/'
    }
};
...
gulp.task('build:nominjs', function () {
    return gulp.src([path.src.js, path.src.libsJs])
        // .pipe(include()).on('error', console.error) 
        .pipe(fileInclude({prefix: '@@', basepath: '@file'})).on('error', console.error) 
        .pipe(sourcemaps.init())
...
gulp.task('copyIndexjs:js', function () {
    return gulp.src([path.src.js])
        .pipe(gulp.dest(path.buildForSite.js)) 
});

gulp.task('copyDepartmentsMapjs:js', function () {
    return gulp.src([path.src.departmentsMapjs])
        .pipe(gulp.dest(path.buildForSite.js)) 
});

gulp.task('buildForSite:js', function () {
    return gulp.src([path.src.js, path.src.libsJs])
        // .pipe(include()).on('error', console.error) 
        .pipe(fileInclude({prefix: '@@', basepath: '@file'})).on('error', console.error) 
        .pipe(sourcemaps.init()) 
...
gulp.task('buildProd:nominjs', function () {
    return gulp.src([path.src.js, path.src.libsJs])
        .pipe(fileInclude({prefix: '@@', basepath: '@file'})).on('error', console.error) 
        .pipe(babel() ).on('error', function(err) {
            notify({ title: 'js:buildProd task error!' }).write(err.message);
  ...
  return gulp.src(['production/css/main.min.css',path.src.purgingcss])
       .pipe(
            purgecss({
                ...
                whitelistPatterns: [/\bjs_/, /active/, /\bslick-/, /\bicon-/, /\bchosen-/, /\bcbox/, /colorbox/, /\blombard/, /\bgm/, /\bwhite-theme/, /\bui-/, /home-page$/, /collapsed$/, /\btcnstk/, /\bpage-preloader/, /links-section/, /link-btn/, /page-links/, /add-to-cart--with-guaranty/, /sticker/, /security_input/],
                whitelistPatternsChildren: [/open$/, /fav-td$/, /chosen-container$/, /link-category$/]
            })
        )
...
gulp.task('build:dev', gulp.series('clean:build', gulp.parallel(
    'build:html','build:js', 'build:nominjs', 'copy:js','build:css','copy:css','build:fonts','build:image','copy_to_root:dev'
    )));
...
gulp.task('watchForSite', function() {
    gulp.watch( path.watch.js, gulp.series('copyIndexjs:js', 'copyDepartmentsMapjs:js', 'buildForSite:js', 'reload') );
});
